...
class VivePoseMapping:
    ...
    def __init__(
        ...
        rospy.Subscriber('/'+self.CONTROLLER_SIDE+'/target_collision', Bool, self.__collsion_callback)
    ...
    def publish_compensate_pose(self):
        ...
        if diff > 0.1:
            ...
            elif self.rightdown or self.leftdown:
                if distance[2] > 0:
                    arm_status_flag = True
                else:
                    arm_status_flag = False
            ...
            if self.CONTROLLER_SIDE == "right" and self.collsion:
                rospy.logwarn("collsion: %s", self.collsion)
            ...
            if arm_status_flag:
                ...
                if self.CONTROLLER_SIDE == "right":
                    if compensated_input_pose['position'][0] < 0.62 and compensated_input_pose['position'][0] > 0.295 and compensated_input_pose['position'][1] > -0.5 and compensated_input_pose['position'][1]<-0.09:
                        overshooting = False
                        #rospy.logwarn("not overshooting")
                    
                        #rospy.logwarn("current position: %s", compensated_input_pose['position'])
