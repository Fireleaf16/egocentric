...
class videoThread(threading.Thread):
    ...
    def get_frame_coordinates(self, base_frame, target_frame):
        try:
            trans = self.tfBuffer.lookup_transform(base_frame, target_frame, rospy.Time())
            # Assuming you have a function to convert 3D coordinates to 2D screen coordinates
    ...
    def convert_3d_to_2d(self, translation):
        ...
        # Implement the conversion based on your camera projection or a simple scaling
        x = translation.x * 45  # Example conversion, adjust accordingly
        y = translation.y * 50
        z = translation.z * 35  # Example conversion, adjust accordingly
        return x, y, z
    ...
    def draw_warnings(self, frame):
        ...
        if self.rightup and self.flash_counter < self.warning_flash_interval:
            cv2.putText(frame, 'Up', (self.right_text_pos[0], self.right_text_pos[1]+30), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 255), 2, cv2.LINE_AA)
        elif self.rightdown and self.flash_counter < self.warning_flash_interval:
            cv2.putText(frame, 'Down', (self.right_text_pos[0], self.right_text_pos[1]+30), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 255), 2, cv2.LINE_AA)
        
        if self.leftup and self.flash_counter < self.warning_flash_interval:
            cv2.putText(frame, 'Up', (self.left_text_pos[0], self.left_text_pos[1]+30), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 255), 2, cv2.LINE_AA)
        elif self.leftdown and self.flash_counter < self.warning_flash_interval:
            cv2.putText(frame, 'Down', (self.left_text_pos[0], self.left_text_pos[1]+30), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 255), 2, cv2.LINE_AA)

        if self.right_arm_warnings and self.flash_counter < self.warning_flash_interval:
            cv2.putText(frame, 'Right', (self.right_text_pos[0], self.right_text_pos[1]-30), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 255), 2, cv2.LINE_AA)
        elif self.right_forward and self.flash_counter < self.warning_flash_interval:
            cv2.putText(frame, 'Forward', (self.right_text_pos[0], self.right_text_pos[1]-30), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 255), 2, cv2.LINE_AA)
        elif self.right_shrink_warnings and self.flash_counter < self.warning_flash_interval:
            cv2.putText(frame, 'Backward', (self.right_text_pos[0], self.right_text_pos[1]-30), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 255), 2, cv2.LINE_AA)
        elif self.right_cross and self.flash_counter < self.warning_flash_interval:
            cv2.putText(frame, 'Left', (self.right_text_pos[0], self.right_text_pos[1]-30), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 255), 2, cv2.LINE_AA)
        
        if self.left_forward and self.flash_counter < self.warning_flash_interval:
            cv2.putText(frame, 'Forward', (self.left_text_pos[0], self.left_text_pos[1]-30), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 255), 2, cv2.LINE_AA)
        elif self.left_shrink_warnings and self.flash_counter < self.warning_flash_interval:
            cv2.putText(frame, 'Backward', (self.left_text_pos[0], self.left_text_pos[1]-30), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 255), 2, cv2.LINE_AA)
        elif self.left_arm_warnings and self.flash_counter < self.warning_flash_interval:
            cv2.putText(frame, 'Left', (self.left_text_pos[0], self.left_text_pos[1]-30), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 255), 2, cv2.LINE_AA)
        elif self.left_cross and self.flash_counter < self.warning_flash_interval:
            cv2.putText(frame, 'Right', (self.left_text_pos[0], self.left_text_pos[1]-30), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 255), 2, cv2.LINE_AA)
    ...
    def run(self):
        ...
        while not rospy.is_shutdown():
            ...
            side_adjust = 25
            top_down_bar_length = 15
            top_adjust_y = -2
            down_adjust_y = 32
            ##------------- draw top and bottom warnings -------------##
            '''
            if self.rightup or self.leftup:
                cv2.line(frame_rs1, (bar_position_x-side_adjust, line_position_y-top_adjust_y), (bar_position_x -side_adjust+ top_down_bar_length, line_position_y-top_adjust_y),red, 2)
                cv2.line(frame_rs1, (bar_position_x+side_adjust, line_position_y-top_adjust_y), (bar_position_x +side_adjust+ top_down_bar_length, line_position_y-top_adjust_y), red, 2)
            else:
                cv2.line(frame_rs1, (bar_position_x-side_adjust, line_position_y-top_adjust_y), (bar_position_x -side_adjust+ top_down_bar_length, line_position_y-top_adjust_y), green, 2)
                cv2.line(frame_rs1, (bar_position_x+side_adjust, line_position_y-top_adjust_y), (bar_position_x +side_adjust+ top_down_bar_length, line_position_y-top_adjust_y), green, 2)

            if self.rightdown or self.leftdown:
                cv2.line(frame_rs1, (bar_position_x-side_adjust, line_position_y+down_adjust_y), (bar_position_x -side_adjust+ top_down_bar_length, line_position_y+down_adjust_y), red, 2)
                cv2.line(frame_rs1, (bar_position_x+side_adjust, line_position_y+down_adjust_y), (bar_position_x +side_adjust+ top_down_bar_length, line_position_y+down_adjust_y), red, 2)
            else:
                cv2.line(frame_rs1, (bar_position_x-side_adjust, line_position_y+down_adjust_y), (bar_position_x -side_adjust+ top_down_bar_length, line_position_y+down_adjust_y), green, 2)
                cv2.line(frame_rs1, (bar_position_x+side_adjust, line_position_y+down_adjust_y), (bar_position_x +side_adjust+ top_down_bar_length, line_position_y+down_adjust_y), green, 2)
            
            '''
            if self.rightup or self.leftup:
                self.draw_dashed_line(frame_rs1, (bar_position_x-side_adjust+5, line_position_y-top_adjust_y), (bar_position_x-side_adjust+top_down_bar_length+5, line_position_y-top_adjust_y), red, thickness=2, dash_length=3, gap_length=3)
                self.draw_dashed_line(frame_rs1, (bar_position_x+side_adjust, line_position_y-top_adjust_y), (bar_position_x+side_adjust+top_down_bar_length, line_position_y-top_adjust_y), red, thickness=2, dash_length=3, gap_length=3)
            else:
                self.draw_dashed_line(frame_rs1, (bar_position_x-side_adjust+5, line_position_y-top_adjust_y), (bar_position_x-side_adjust+top_down_bar_length+5, line_position_y-top_adjust_y), (204,206,144), thickness=2, dash_length=3, gap_length=3)
                self.draw_dashed_line(frame_rs1, (bar_position_x+side_adjust, line_position_y-top_adjust_y), (bar_position_x+side_adjust+top_down_bar_length, line_position_y-top_adjust_y), (204,206,144), thickness=2, dash_length=3, gap_length=3)
                

            if self.rightdown or self.leftdown:
                self.draw_dashed_line(frame_rs1, (bar_position_x-side_adjust+5, line_position_y+down_adjust_y), (bar_position_x-side_adjust+top_down_bar_length+5, line_position_y+down_adjust_y), red, thickness=2, dash_length=3, gap_length=3)
                self.draw_dashed_line(frame_rs1, (bar_position_x+side_adjust, line_position_y+down_adjust_y), (bar_position_x+side_adjust+top_down_bar_length, line_position_y+down_adjust_y), red, thickness=2, dash_length=3, gap_length=3)
            else:
                self.draw_dashed_line(frame_rs1, (bar_position_x-side_adjust+5, line_position_y+down_adjust_y), (bar_position_x-side_adjust+top_down_bar_length+5, line_position_y+down_adjust_y),(204,206,144), thickness=2, dash_length=3, gap_length=3)
                self.draw_dashed_line(frame_rs1, (bar_position_x+side_adjust, line_position_y+down_adjust_y), (bar_position_x+side_adjust+top_down_bar_length, line_position_y+down_adjust_y), (204,206,144), thickness=2, dash_length=3, gap_length=3)
            ...
            ##--------------workspace boundary----------##
            foward_limit = int(0.74*50)
            side_limit = int(0.5*50)+1

            if self.right_forward or self.left_forward:
                self.draw_dashed_line(frame_rs1, (350-side_limit, 375-foward_limit), (350+side_limit, 375-foward_limit), (0, 0, 255), thickness=2, dash_length=2, gap_length=2)
            else:
                self.draw_dashed_line(frame_rs1, (350-side_limit, 375-foward_limit), (350+side_limit, 375-foward_limit), (204,206,144), thickness=2, dash_length=2, gap_length=2)

            if self.right_arm_warnings:
                self.draw_dashed_line(frame_rs1, (350+side_limit, 375-foward_limit), (350+side_limit, 355), (0, 0, 255), thickness=2, dash_length=2, gap_length=2)
                self.draw_dashed_line(frame_rs1, (bar_position_x+side_adjust+int(top_down_bar_length/2), line_position_y-top_adjust_y+4), (bar_position_x+side_adjust+int(top_down_bar_length/2), line_position_y+down_adjust_y), red, thickness=2, dash_length=3, gap_length=3)
            else:
                self.draw_dashed_line(frame_rs1, (350+side_limit, 375-foward_limit), (350+side_limit, 355), (204,206,144), thickness=2, dash_length=2, gap_length=2)
                self.draw_dashed_line(frame_rs1, (bar_position_x+side_adjust+int(top_down_bar_length/2), line_position_y-top_adjust_y+4), (bar_position_x+side_adjust+int(top_down_bar_length/2), line_position_y+down_adjust_y), (204,206,144), thickness=2, dash_length=3, gap_length=3)
               

            if self.left_arm_warnings:
                self.draw_dashed_line(frame_rs1, (350-side_limit, 375-foward_limit), (350-side_limit, 355), (0, 0, 255), thickness=2, dash_length=2, gap_length=2)
                self.draw_dashed_line(frame_rs1, (bar_position_x-side_adjust+int(top_down_bar_length/2), line_position_y-top_adjust_y+4), (bar_position_x-side_adjust+int(top_down_bar_length/2), line_position_y+down_adjust_y), red, thickness=2, dash_length=3, gap_length=3)
            else:
                self.draw_dashed_line(frame_rs1, (350-side_limit, 375-foward_limit), (350-side_limit, 355),(204,206,144), thickness=2, dash_length=2, gap_length=2)
            ...
            if (self.right_shrink_warnings or self.left_shrink_warnings) and (self.right_act or self.left_act):
                self.draw_dashed_line(frame_rs1, (350-side_limit, 355), (350+side_limit, 355), (0, 0, 255), thickness=2, dash_length=2, gap_length=2)
            else:
                self.draw_dashed_line(frame_rs1, (350-side_limit, 355), (350+side_limit, 355), (204,206,144), thickness=2, dash_length=2, gap_length=2)


            if self.right_cross:
                self.draw_dashed_line(frame_rs1, (350, 375-foward_limit), (350, 355), (0, 0, 255), thickness=2, dash_length=3, gap_length=3)

            if self.left_cross:
                self.draw_dashed_line(frame_rs1, (350, 375-foward_limit), (350, 355), (0, 0, 255), thickness=2, dash_length=2, gap_length=2)

            if (not self.right_cross) and (not self.left_cross):
                self.draw_dashed_line(frame_rs1, (350, 375-foward_limit), (350, 355), (204,206,144), thickness=2, dash_length=2, gap_length=2)
            ...
            # Draw the limit arrow
            '''
            if (self.right_shrink_warnings or self.left_shrink_warnings) and (self.right_act or self.left_act):
                start_point = (350, 375)  # Starting point of the arrow
                end_point = (350, 395)  # Ending point of the arrow
            ...
            elif self.right_forward or self.left_forward:
                start_point = (350, 375)  # Starting point of the arrow
                end_point = (350, 355)  # Ending point of the arrow
                color = (0, 0, 255)  # Red color for the arrow
                thickness = 2  # Thickness of the arrow
                cv2.arrowedLine(frame_rs1, start_point, end_point, color, thickness, tipLength=0.3)
            elif self.right_arm_warnings or self.left_cross:
                start_point = (350, 375)  # Starting point of the arrow
                end_point = (370, 375)  # Ending point of the arrow
                color = (0, 0, 255)  # Red color for the arrow
                thickness = 2  # Thickness of the arrow
                cv2.arrowedLine(frame_rs1, start_point, end_point, color, thickness, tipLength=0.3)
            elif self.left_arm_warnings or self.right_cross:
                start_point = (350, 375)  # Starting point of the arrow
                end_point = (330, 375)  # Ending point of the arrow
                color = (0, 0, 255)  # Red color for the arrow
                thickness = 2  # Thickness of the arrow
                cv2.arrowedLine(frame_rs1, start_point, end_point, color, thickness, tipLength=0.3)
            ...
            if (self.rightdown or self.leftdown) and self.torso_height > 0.075: 
                start_point = (253, line_position_y +15)  # Starting point of the 
                end_point = (253,line_position_y +35)  # Ending point of the arrow
                color = red
                thickness = 2  # Thickness of the arrow
                cv2.arrowedLine(frame_rs1, start_point, end_point, color, thickness, tipLength=0.3)

            if (self.rightup or self.leftup) and self.torso_height < 0.345:
            ...
            if self.head_current_state == -1.0 and self.torso_height > 0.075:
                start_point = (253, line_position_y +15)  # Starting point of the arrow
                end_point = (253,line_position_y +35)  # Ending point of the arrow
                color = red
                thickness = 2  # Thickness of the arrow
                cv2.arrowedLine(frame_rs1, start_point, end_point, color, thickness, tipLength=0.3)
            elif self.head_current_state == 1.0 and self.torso_height < 0.345:
                start_point = (253,line_position_y +5)  # Starting point of the arrow
                end_point = (253,line_position_y -15)  # Ending point of the arrow
                color = red
                thickness = 2  # Thickness of the arrow
