...
The TiagoArmEEPublisher class is used to publish the end-effector tf transforms for the right and left arms.
compensate_translation is used to store the translation of the arm_1_link frame relative to the torso_lift_link frame.
collsion is used to store the translation of the gripper frame relative to the odom frame.
elbow_pos is used to store the translation of the arm_4_link frame relative to the odom frame.
...
from geometry_msgs.msg import Pose, Twist, PoseStamped, TransformStamped,Point
...
class TiagoArmEEChecker():
    def __init__(self,
        ...
        self.target_collosion_pub = rospy.Publisher('/'+self.controller_side+'/target_collision',Point, queue_size = 1)
        ...
        while not rospy.is_shutdown():
            ...
            try:
                ...
                if available:
                    ...
                    point_msg = Point()
                    point_msg.x = next_target_collision['position'][0]
                    point_msg.y = next_target_collision['position'][1]
                    point_msg.z = next_target_collision['position'][2]
                    '''
                    ...
                    self.target_collosion_pub.publish(point_msg)
