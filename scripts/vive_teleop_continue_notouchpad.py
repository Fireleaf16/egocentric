...
class JoystickTeleop():
    def __init__(self, sim=True):
        ...
        self.head_yaw_duration = 0.1  # seconds
        self.head_yaw_start_time = None
        self.head_pitch_up_threshold = 0.0
    ...
    def __right_arm_collision_callback(self, msg):
            ...
            if self.right_collision:
                rospy.logwarn("right arm collision detected")
    ...
    def __head_motion_callback(self, msg):
        ...
        if self.head_joint_yaw > self.head_yaw_threshold:
            ...
            self.head_yaw_exceeded = True  # Indicate that the duration has been exceeded
            ...
            self.head_yaw_start_time = None  # Reset if within threshold
            self.head_yaw_exceeded = False  # Reset the exceeded flag
    ...
    def __apply_movement(self):
        ...
        if (self.arm_state['right_shrink'] or self.arm_state['left_shrink']) and (self.left_act or self.right_act):
            self.__move_base_and_torso((-1, 0, 0))
        elif (self.arm_state['right_forward'] or self.arm_state['left_forward']) and (self.left_act or self.right_act):
            self.__move_base_and_torso((1, 0, 0))
        elif (self.arm_state['right_extended'] or self.arm_state['left_cross']) and (self.left_act or self.right_act):
            self.__move_base_and_torso((0, -1, 0))
        elif (self.arm_state['left_extended'] or self.arm_state['right_cross']) and (self.left_act or self.right_act):
            self.__move_base_and_torso((0, 1, 0))
        else:
